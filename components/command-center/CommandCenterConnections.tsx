  ...
  return (
    ...
    >
      ...
      {planetPositions.map((pos) => {
        ...
        const curveOffset = Math.sin((pos.x + pos.y) * 0.01) * 18;
        const ctrlX = midX - (targetY - cy) * 0.05 + curveOffset;
        const ctrlY = midY + (targetX - cx) * 0.05;

        const pathD = `M ${cx} ${cy} Q ${ctrlX} ${ctrlY} ${targetX} ${targetY}`;
